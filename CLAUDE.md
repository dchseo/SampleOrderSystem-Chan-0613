...
- [docs/CLEAN_CODE_REVIEW.md](docs/CLEAN_CODE_REVIEW.md) — Phase 7에서 진행한 OOP/SOLID 관점의
  구조적 트레이드오프 검토. 실제로 리팩토링한 지점과, 검토 후 의도적으로 그대로 둔 지점(SRP/ISP/
  계층 분리 등)을 근거와 함께 정리했다.
- [docs/COMMIT_HISTORY.md](docs/COMMIT_HISTORY.md) — 전체 커밋 이력을 해시/제목/반영 내용 표로
  정리한 문서.
